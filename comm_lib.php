...
class CommLib extends BitBase {
	...
	function accept_page($received_page_id) {
		...
		$result = $this->query($query,array((int)$received_page_id));
		return true;
	}
	...
	function accept_article($received_article_id, $topic) {
		...
		$result = $this->query($query,array((int)$received_article_id));
		return true;
	}
	...
	function list_received_articles($offset, $maxRecords, $sort_mode = 'publish_date_desc', $find) {
		...
		$query = "select * from `".BIT_DB_PREFIX."tiki_received_articles` $mid order by ".$this->convert_sortmode($sort_mode);
	...
	function rename_received_page($received_page_id, $name) {
		...
		$result = $this->query($query,array($name,(int)$received_page_id));
	}
	...
	function update_received_page($received_page_id, $page_name, $data, $comment) {
		...
		$result = $this->query($query,array($page_name,$data,$comment,(int)$received_page_id));
	}
    ...
    $query.= " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		$result = $this->query($query,array((int)$now,$site,$user,$title,$author_name,(int)$size,$use_image,$image_name,$image_type,$image_size,
	...
	function receive_page($page_name, $data, $comment, $site, $user, $description) {
		...
		$result = $this->query($query,array($page_name,$site,$user));
		// Now insert the page
		...
		$result = $this->query($query,array($page_name,$data,$comment,$site,$user,(int)$now,$description));
	}
