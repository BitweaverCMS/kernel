 ...
 *
 * @package kernel
 * @version $Header: /cvsroot/bitweaver/_bit_kernel/Attic/admin_lib.php,v 1.1.1.1.2.6 2005/08/07 13:27:38 lsces Exp $
 */
...
class AdminLib extends BitBase {
	...
	function list_dsn($offset, $maxRecords, $sort_mode, $find) {
		...
		$query = "select * from `".BIT_DB_PREFIX."tiki_dsn` $mid order by ".$this->getDb()->convert_sortmode($sort_mode);
	...
	function replace_dsn($dsn_id, $dsn, $name) {
		...
		$this->getDb()->query($query,array($perm_name));
		$query = "insert into `".BIT_DB_PREFIX."users_permissions`(`perm_name`,`perm_desc`,`type`,`level`) values
    			(?,?,?,?)";
		$this->getDb()->query($query,array($perm_name,'Can use dsn $dsn','dsn','editor'));
		return true;
	}
	...
	function remove_dsn($dsn_id) {
		...
		$this->getDb()->query($query,array($dsn_id));
		return true;
	}
	...
	function get_dsn($dsn_id) {
		...
		$result = $this->getDb()->query($query,array($dsn_id));
	...
	function list_extwiki($offset, $maxRecords, $sort_mode, $find) {
		...
		$query = "select * from `".BIT_DB_PREFIX."tiki_extwiki` $mid order by ".$this->getDb()->convert_sortmode($sort_mode);
	...
	function replace_extwiki($extwiki_id, $extwiki, $name) {
		...
		if ($extwiki_id) {
			...
			$result = $this->getDb()->query($query,array($extwiki,$name,$extwiki_id));
		} else {
		...
		$query = "insert into `".BIT_DB_PREFIX."users_permissions`(`perm_name`,`perm_desc`,`type`,`level`) values
    			(?,?,?,?)";
		$this->getDb()->query($query,array($perm_name,'Can use extwiki $extwiki','extwiki','editor'));
		return true;
	}
	...
	function remove_extwiki($extwiki_id) {
		...
		$this->getDb()->query($query,array($extwiki_id));
		return true;
	}
	...
	function get_extwiki($extwiki_id) {
		...
		$result = $this->getDb()->query($query,array($extwiki_id));
	...
	function tag_exists($tag) {
		...
		$result = $this->getDb()->query($query,array($tag));
	...
	function remove_tag($tagname) {
		...
		$result = $this->getDb()->query($query,array($tagname));
		$action = "removed tag: $tagname";
		$t = date("U");
		$homePageId = $this->getDb()->getOne( "SELECT `page_id` from `".BIT_DB_PREFIX."tiki_pages` tp INNER JOIN `".BIT_DB_PREFIX."tiki_content` tc ON(tp.`content_id`=tc.`content_id`) WHERE tc.`title`=?", array( $wikiHomePage ) );
		$query = "insert into `".BIT_DB_PREFIX."tiki_actionlog` (`page_id`, `action`, `page_name`, `last_modified`, `user_id`, `ip`, `comment`) values ( ?,?,?,?,?,?,? )";
		$result = $this->getDb()->query($query,array($homePageId, $action,$wikiHomePage,$t,$gBitUser->mUserId,$_SERVER["REMOTE_ADDR"],''));
	...
	function create_tag($tagname, $comment = '') {
		...
		while ($res = $result->fetchRow()) {
			...
			$this->getDb()->query($query,array($tagname,$res["page_id"]));
			$query = "insert into `".BIT_DB_PREFIX."tiki_tags`(`page_id`,`tag_name`,`page_name`,`hits`,`data`,`last_modified`,`comment`,`version`,`user_id`,`ip`,`flag`,`description`)
                		values(?,?,?,?,?,?,?,?,?,?,?,?)";
			$result2 = $this->getDb()->query($query,array($res["page_id"],$tagname,$res["title"],$res["hits"],$data,$res["last_modified"],$res["comment"],$res["version"],$res["user_id"],$res["ip"],$res["flag"],$description));
		}
		...
		$t = date("U");
		$query = "insert into `".BIT_DB_PREFIX."tiki_actionlog`(`page_id`,`action`,`page_name`,`last_modified`,`user_id`,`ip`,`comment`) values(?,?,?,?,?,?,?)";
		$result = $this->getDb()->query($query,array($homePageId,$action,$wikiHomePage,$t,$gBitUser->mUserId,$_SERVER["REMOTE_ADDR"],$comment));
		$this->getDb()->CompleteTrans();
		return true;
	}
